...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (post.status === "PUBLICADO" && post.publish_id) {
      const published = await metaGet(`https://graph.facebook.com/${GRAPH_VERSION}/${post.publish_id}?fields=id,permalink,timestamp&access_token=${encodeURIComponent(token)}`);
    ...
    const statusRes = await metaGet(`https://graph.facebook.com/${GRAPH_VERSION}/${post.container_id}?fields=status_code,status&access_token=${encodeURIComponent(token)}`);
